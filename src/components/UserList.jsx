...
// Centralized Avatar Map to match across components
const AVATAR_MAP = {
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-30 shadow-sm backdrop-blur-md bg-white/90">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-6">
            ...
            <button 
              onClick={logout} 
              className="bg-rose-50 border border-rose-200 text-rose-600 hover:bg-rose-100 px-4 py-2 rounded-xl text-sm font-semibold tracking-wide transition-all duration-200 active:scale-95 shadow-sm"
            >
              Sign Out
            </button>
          </div>
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 flex-1">
              <div className="relative">
                <input 
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-3 pr-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all placeholder-slate-400 font-medium" 
                  value={search} 
              ...
              <select 
                className="bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-sm text-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-medium cursor-pointer" 
                value={genderFilter} 
            ...
            <div className="lg:self-center">
              <button 
                ...
                className="w-full lg:w-auto inline-flex items-center justify-center bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-sm px-5 py-2.5 rounded-xl transition-all duration-200 active:scale-95 shadow-sm gap-2"
              >
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/70 border-b border-slate-200">
                  <th className="py-3.5 px-6 text-xs font-bold uppercase tracking-wider text-slate-500">Profile</th>
                  <th className="py-3.5 px-6 text-xs font-bold uppercase tracking-wider text-slate-500">Email Address</th>
                    ...
                    <tr key={user.id} className="hover:bg-slate-50/50 transition-colors group">
                      ...
                      <td className="py-4 px-6 whitespace-nowrap text-right space-x-2">
                        <button 
                          ...
                          className="bg-white border border-slate-200 hover:border-indigo-300 text-indigo-600 hover:bg-indigo-50 px-3 py-1.5 rounded-lg text-xs font-bold transition-all duration-150"
                        >
                          Edit
                        </button>
                        <button 
                          onClick={() => handleDelete(user.id)} 
                          className="bg-white border border-slate-200 hover:border-rose-300 text-rose-600 hover:bg-rose-50 px-3 py-1.5 rounded-lg text-xs font-bold transition-all duration-150"
                        >
                          Delete
                        </button>
                      </td>
            ...
            <form onSubmit={handleUpdate} className="space-y-4">
              <div>
                ...
                <input 
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-medium" 
                  required 
                />
              </div>
              ...
              <div>
                ...
                <input 
                  type="password" 
                  placeholder="Leave completely blank to retain current" 
                  onChange={e => setEditUser({...editUser, password: e.target.value})} 
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-medium placeholder-slate-400" 
                />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1.5">Gender Orientation</label>
                  <select 
                    value={editUser.gender} 
                    onChange={e => setEditUser({...editUser, gender: e.target.value})} 
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-medium cursor-pointer"
                  >
                    <option value="male">Male</option>
                    <option value="female">Female</option>
                    <option value="others">Others</option>
                ...
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1.5">Registered City</label>
                  <select 
                    value={editUser.city} 
              ...
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">Display Avatar Graphic</label>
                <div className="grid grid-cols-4 gap-3">
                  {[1, 2, 3, 4].map(num => (
                    <label key={num} className="cursor-pointer group relative flex flex-col items-center">
                      <div className={`w-14 h-14 rounded-xl object-cover bg-slate-100 p-1 border-2 transition-all flex items-center justify-center ${
                        editUser.selected_image == num 
                      ...
                      <input 
                        ...
                        name="edit_image" 
                        value={num} 
                        checked={editUser.selected_image == num} 
                        onChange={() => setEditUser({...editUser, selected_image: num.toString()})} 
                        className="hidden" 
                      />
                    </label>
                  ))}
              ...
              <div className="flex justify-end gap-2 pt-3 border-t border-slate-100 mt-6">
                <button 
                  ...
                  className="bg-white border border-slate-200 text-slate-500 hover:bg-slate-50 px-4 py-2 rounded-xl text-sm font-semibold transition"
                >
                  Discard
                </button>
                <button 
                  type="submit" 
                  className="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2 rounded-xl text-sm font-semibold transition shadow-sm active:scale-95"
                >
                  Save Changes
                </button>
              </div>
